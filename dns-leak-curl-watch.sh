...
stop_monitor() {
  if [ -f "$PIDFILE" ]; then
    ...
    if ps -p "$PID" > /dev/null 2>&1; then
      ...
      echo "⚠️ PID 文件存在但进程未运行，正在清理残留文件"
      rm -f "$PIDFILE"
    fi
  else
    echo "⚠️ 未找到运行中的监控进程"
